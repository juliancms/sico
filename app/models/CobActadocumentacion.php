...
class CobActadocumentacion extends \Phalcon\Mvc\Model
{
    ...
    public function cargarBeneficiarios($carga, $modalidades, $id_verificacion)
    {
      ...
      $db->query("DELETE FROM $tabla_mat WHERE CONCAT_WS('-',id_contrato,numDocumento) IN (SELECT CONCAT_WS('-',id_contrato,numDocumento) FROM cob_actadocumentacion_persona WHEER 1)");
    	...
    	$db->query("INSERT IGNORE INTO cob_actadocumentacion_persona (id_actadocumentacion, id_verificacion, grupo, id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, beneficiarioTelefono, beneficiarioCelular) SELECT (SELECT id_actadocumentacion FROM cob_actadocumentacion WHERE cob_actadocumentacion.id_sede_contrato = $tabla_mat.id_sede_contrato AND cob_actadocumentacion.id_verificacion = $id_verificacion), $id_verificacion, grupo, id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, telefonoBeneficiario, celularBeneficiario FROM $tabla_mat");
