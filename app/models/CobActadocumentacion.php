...
class CobActadocumentacion extends \Phalcon\Mvc\Model
{
    ...
    public function cargarBeneficiarios($carga, $modalidades, $id_verificacion)
    {
      ...
      $db->query("DELETE FROM $tabla_mat WHERE CONCAT(id_contrato,numDocumento) IN (SELECT CONCAT(id_contrato,numDocumento) FROM cob_actadocumentacion_persona WHERE 1)");
