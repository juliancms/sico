...
class CobActaconteo extends \Phalcon\Mvc\Model
{
    ...
    public function generarActasR1($cob_periodo, $carga, $modalidades, $facturacion) {
        ...
        $db->query("INSERT IGNORE INTO cob_actaconteo_persona (id_actaconteo, id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, id_grupo, grupo) SELECT (SELECT id_actaconteo FROM cob_actaconteo WHERE cob_actaconteo.id_sede_contrato = $tabla_mat.id_sede_contrato AND cob_actaconteo.id_periodo = $cob_periodo->id_periodo AND cob_actaconteo.recorrido = 1), id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, id_grupo, grupo FROM $tabla_mat");
        if($facturacion == 1){
        	//$tabla_sed = "s" . $timestamp->getTimestamp();
        	//$db->query("CREATE TEMPORARY TABLE $tabla_sed (id_sede_contrato BIGINT, cuposSede INT, cuposSostenibilidad INT, cuposAmpliacion INT, cuposTotal INT) CHARACTER SET utf8 COLLATE utf8_bin");
        	...
        	$db->query("INSERT IGNORE INTO cob_actaconteo_persona_facturacion (id_periodo, id_sede_contrato, id_contrato, id_sede, id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, id_grupo, grupo, fechaInicioAtencion, fechaRegistro, fechaRetiro, fechaNacimiento, peso, estatura, fechaControl) SELECT $cob_periodo->id_periodo, id_sede_contrato, id_contrato, id_sede, id_persona, numDocumento, primerNombre, segundoNombre, primerApellido, segundoApellido, id_grupo, grupo, fechaInicioAtencion, fechaRegistro, fechaRetiro, fechaNacimiento, peso, estatura, fechaControl FROM $tabla_mat");
        }
    ...
    public function generarActa($id_actaconteo, $cob_periodo){
    	...
    	$acta->getCobActaconteoPersona(['order' => 'grupo asc']);
    	if(!$acta || $acta == NULL){
    		return FALSE;
    	}
