...
<table class="table table-bordered table-hover" id="permisos_lista">
    ...
    {% for permiso in permisos %}
        <tr data-estado="{{ permiso.estado }}" class="bg-{{ permiso.getEstadoStyle() }}">
            <td><a rel="tooltip" title="Ver Detalles del Permiso" href="bc_permiso/permiso/{{ permiso.id_permiso }}"><?php if (strtotime($permiso->fecha) > $fecha_limite && $permiso->estado < 5){ ?><a href="#eliminar_elemento" rel="tooltip" title="Anular" class="eliminar_fila" data-id = "{{ permiso.id_permiso }}" data-toggle = "modal" id="{{ url("bc_permiso/eliminar/"~permiso.id_permiso) }}"><i class="glyphicon glyphicon-remove"></i></a> <?php } ?>{{ permiso.id_permiso }}</a></td>
            <td><a rel="tooltip" title="Ver Detalles del Permiso" href="bc_permiso/permiso/{{ permiso.id_permiso }}">{{ permiso.getEstado() }}</a></td>
